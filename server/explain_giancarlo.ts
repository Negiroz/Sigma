...
async function main() {
    const agent = await prisma.employee.findFirst({
        where: { name: { contains: 'Giancarlo' } }
    });

    if (!agent) {
        console.log('Giancarlo not found');
        return;
    }

    console.log(`\n=== REPORTE DE XP PARA: ${agent.name} (Role: ${agent.role}) ===\n`);
    ...
    for (const [monthYear, mList] of monthMap.entries()) {
        ...
        // Use current role for scoring logic
        ...
        console.log(`  - Prospectos: ${sum.prospects}`);
        console.log(`  - Tickets Soporte: ${sum.supportTickets}`);
        console.log(`  - Tareas Hechas: ${sum.tasksDone}`);
        console.log(`  - Puntos Versus: ${sum.versusPoints}`);
        console.log(`  => PUNTAJE DEL MES: ${monthlyScore}\n`);

        cumulativeComputedXp += monthlyScore;
    }

    console.log(`Si sumamos el puntaje de todos sus meses trabajados da: ${cumulativeComputedXp}`);
    console.log(`*(Nota: currentXp puede diferir de la suma si hubo decaimiento (impuestos), bonos manuales, o si su rol cambió en el pasado)*`);
}
